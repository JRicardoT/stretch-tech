...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async() => {
      const res = await fetch(`https://api.spoonacular.com/recipes/complexSearch?${cuisineTypeEndPoint}${intoleranceEndPoint}${dietRestrictionEndPoint}${typeEndPoint}&apiKey=5c9e42f2cb484754ae6408f57b5b9e18`)
  ...
  const randomSearch = (event: any) => {
    event?.preventDefault();
    const fetchData = async() => {
      const res = await fetch(`https://api.spoonacular.com/recipes/random?number=10&apiKey=5c9e42f2cb484754ae6408f57b5b9e18`)
      const resJson = await res.json()
